...
EOL
   if [ -d "${BSP_DIR}/sources/meta-alvium-bitstream" ]; then

        if [ -z $(grep "meta-alvium-bitstream" ${BBLAYERS_FILE}) ]; then
            echo "BBLAYERS += \"\${BSPDIR}/sources/meta-alvium-bitstream\"" >> ${BBLAYERS_FILE}
        fi
    else
        echo "Warning: meta-alvium-bitstream not found! Image will not contain any bitstream!"
    fi
